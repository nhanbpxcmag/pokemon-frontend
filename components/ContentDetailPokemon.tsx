...
import { getPokemon, getUrlPokemon } from "../api/pokemon";
...
interface PropsContent {
  selectPokemonDetail: Pokemon;
  idPokemonDetail: string;
}

const ContentDetailPokemon: React.FC = () => {
  const { idPokemonDetail, setIdPokemonDetail, removeIdPokemonDetail } = useApp();

  const { data: selectPokemonDetail, isFetching } = useQuery(["pokemon", getUrlPokemon(idPokemonDetail)], async () => {
    if (idPokemonDetail) return (await getPokemon(idPokemonDetail)).data;
    return undefined;
  });
  return (
    <>
      {isFetching && <LoadingPage isLoading={true} />}
      {selectPokemonDetail !== undefined && <Content selectPokemonDetail={selectPokemonDetail} idPokemonDetail={idPokemonDetail} />}
    </>
  );
};

const Content: React.FC<PropsContent> = ({ selectPokemonDetail, idPokemonDetail }) => {
  const {
    id,
    sprites,
    name,
  ...
  const { isFetching: isFetchingSpecies, data: dataSpecies } = useQuery(["species", id], async () => {
    if (url_species) return (await axios.get<Species>(url_species)).data;
    return undefined;
  });
